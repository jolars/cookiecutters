...
A collection of [cookiecutter](https://cookiecutter.readthedocs.io/) templates
for scaffolding the author's own project types: Rust, Python, Julia, Python/C++
and R packages; reproducible R projects; and LaTeX articles, posters, and
presentations. There is no application code in this repository—the "source" is
the templates themselves.

Each top-level directory is one independent template: `rust_package/`,
`python_package/`, `python_cpp_package/`, `julia_package/`, `r_package/`,
`r_project/`, `tex_article/`, `tex-poster/`, and `tex_presentation/`. Each
contains a `cookiecutter.json` and a single rendered directory.
...
- Template files live under the rendered directory whose name is a Jinja2
  ...
  Quote these paths in the shell (the `{{ }}` braces are glob/brace-expansion
  bait).
...
- Untracked leftover directories like `{{cookiecutter.slug}}/` or
  `{{cookiecutter.project_name}}/` exist inside some templates from past
  renames. They are git-ignored cruft; the live template is whatever
  `git ls-files` tracks (always the `dir_name`/`slug` dir above). Edit only the
  tracked directory.
- `_copy_without_render` in each `cookiecutter.json` lists globs (PDFs, images,
  compiled objects, and often `.github/*` or `.github/workflows/*`) that are
  copied verbatim. Files matching these are NOT Jinja2-rendered, so cookiecutter
  variables inside them stay literal—keep that in mind when adding such files.
- `python_cpp_package` is the only template using the `jinja2_time` extension
  (for `{% now %}`), so its `year` is computed; other templates hardcode the
  year string and need periodic bumping.
...
`--no-input` uses the defaults from `cookiecutter.json`. For
`python_cpp_package` you also need `pip install jinja2-time` (or run it from an
environment that has it). After a change, regenerate and inspect the output dir
to confirm Jinja2 renders cleanly and filenames came out as expected.
...
generation locally for the template you touched rather than relying on CI to
cover siblings.
...
- Documentation via Sphinx (Python) or Documenter.jl (Julia).
- Panache for Markdown formatting and linting, Versionary for releases, and a
  concise `AGENTS.md` for package templates.
